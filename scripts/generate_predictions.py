...
def prepare_data_for_model(df, features_list):
    ...
    if (
        ...
        df_proc["YearsPerCompany"] = df_proc.apply(
            lambda row: (
                row["TotalWorkingYears"] / row["NumCompaniesWorked"]
                if row["NumCompaniesWorked"] > 0
                else row["TotalWorkingYears"]
            ),
            axis=1,
        ).round(4)
...
def save_predictions_to_postgres(
    df_original: pd.DataFrame, predictions: np.ndarray, db_url: str, table_name: str
):
...
def main():
    ...
    logging.info(
        "🚀 - Iniciando pipeline de geração de predições em lote para PostgreSQL..."
    )
    try:
        df_employees = load_data_from_postgres(POSTGRES_URL, EMPLOYEES_TABLE)
        if df_employees.empty:
            logging.warning(
                "A tabela de funcionários está vazia ou não pôde ser lida. Abortando."
            )
            return
        ...
        save_predictions_to_postgres(
            df_employees, probabilities, POSTGRES_URL, PREDICTIONS_TABLE
        )
        ...
        logging.error(
            f"❌ - Ocorreu um erro inesperado no pipeline: {e}", exc_info=True
        )
