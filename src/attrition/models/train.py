...
# <<< FUNÇÃO AUXILIAR ADICIONADA AQUI >>>
def ensure_dir(file_path):
    """Garante que o diretório de um arquivo exista antes de salvá-lo."""
...
<<<<<<< HEAD
        ...
        if threshold_path:
            ...
            ensure_dir(threshold_path) # <<< CORREÇÃO APLICADA
        ...
        if x_test_out and y_test_out:
            ensure_dir(x_test_out) # <<< CORREÇÃO APLICADA
            X_test.to_csv(x_test_out, index=False)
            
            ensure_dir(y_test_out) # <<< CORREÇÃO APLICADA
