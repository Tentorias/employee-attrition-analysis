...
def run_causal_analysis(
    df: pd.DataFrame, treatment_var: str, outcome_var: str, common_causes: list
):
    """
    ...
    if len(valid_common_causes) != len(common_causes):
        print(
            f"Atenção: Algumas common_causes não foram encontradas no DataFrame e foram removidas: {list(set(common_causes) - set(valid_common_causes))}"
        )
