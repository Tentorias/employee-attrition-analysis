...
def test_cli_run_pipeline_success(tmp_path):
    ...
    sample_data = {
        'Attrition': ['No', 'Yes'] * 20, 'Gender': ['Male', 'Female'] * 20,
        'TotalWorkingYears': range(40), 'NumCompaniesWorked': range(40),
        'MonthlyIncome': range(1000, 1040), 'Department': ['Sales', 'R&D'] * 20
    }
    ...
    result = run_cli([
        "run-pipeline",
        "--raw-data-path", str(raw_data_path),
        "--model-path", str(model_path),
        "--features-path", str(features_path),
        "--params-path", str(params_path),
        "--x-test-path", str(x_test_path),
        "--y-test-path", str(y_test_path),
        "--prod-model-path", str(prod_model_path)
    ])
