...
# Importando a função preprocess do local CORRETO e outras funções de unidade.
...
def test_preprocess_function_logic():
    ...
    assert df_processed["YearsPerCompany"].iloc[1] == 5.0 # 5 / 1 (evitou divisão por zero)

    # 4. Verifica se o One-Hot Encoding para colunas categóricas funcionou
    assert "Department_Sales" in df_processed.columns # 'Research & Development' é a base (drop_first=True)
